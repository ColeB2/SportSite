...
def league_admin_user_info_view(request, user_name, user_pk):
    ...
    return render(request, "league_admin/user_info.html", context)
...
def league_admin_user_edit_perms_view(request, user_name, user_pk):
    ...
    roster_permissions = Permission.objects.filter(content_type__app_label=Roster._meta.app_label,
        content_type__model=Roster._meta.model_name, codename__contains='user')
    user_permissions = Permission.objects.filter(user__pk=user_pk)

    if request.method =="POST":
        form = EditUserRosterPermissionsForm(data=request.POST, selected_user=user_viewed)
        if form.is_valid():
            if form.has_changed():
                form.process()
                messages.success(request, f"{user_viewed}'s permissions updated.")
    ...
    return render(request, "league_admin/user_edit_perms.html", context)
